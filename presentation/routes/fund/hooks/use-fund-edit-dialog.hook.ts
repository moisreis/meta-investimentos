 ...
 *
 * @remarks
 * Tracks the fund being edited, the dialog open state
 * and the form submit status. The row-actions menu
 * triggers `handleOpen` with the target row. On a
 ...
 *
 * @explanation
 * Use inside the route edit dialog component to keep
 * the flow state out of the presentational layer. The
 * target seeds the edit form with the current row
 * values.
 *
 ...
 * @date 2026-09-25
 */
function useFundEditDialog() {
  const ROUTER = useRouter()
  const [TARGET, setTarget] =
    React.useState<FundResponseDTO | null>(null)
