...
import { CHECKING_ACCOUNT_DATATABLE_COLUMN_LABELS } from "@/presentation/routes/checking-account/settings/labels.settings"

interface CheckingAccountDatatableToolbarProps<
...
/**
 * @summary
 * Renders the checking account datatable toolbar.
 *
 * @remarks
 * Composes the shared datatable toolbar with the
 * add-item button, a separator and the edit-columns
 * button. The add-item button opens the checking
 * account add dialog and the edit-columns button
 * toggles the visibility of the hideable columns,
 * resolving each column label through the checking
 * account settings. The optional `filters` slot renders
 * on the left side.
 *
 ...
 *
 * @returns The checking account datatable toolbar.
 *
 * @author Moisés Reis
 *
 * @date 2026-09-25
 */
function CheckingAccountDatatableToolbar<TData extends RowData>({
  table,
  onAddItem,
  filters,
}: CheckingAccountDatatableToolbarProps<TData>) {
  return (
        ...
        <>
          <EntityDatatableEditColumnsButton
            ...
            getColumnLabel={(column) =>
              CHECKING_ACCOUNT_DATATABLE_COLUMN_LABELS[
                column.id
              ] ?? column.id
            }
          />
...
export { CheckingAccountDatatableToolbar }
