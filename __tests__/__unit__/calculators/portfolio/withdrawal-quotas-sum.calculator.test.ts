...
import { calculatePortfolioWithdrawalQuotasSum } from "@/business/calculators/portfolio/withdrawal-quotas-sum.calculator";
...
describe("calculatePortfolioWithdrawalQuotasSum", () => {
  it("returns the sum of multiple quota quantities", () => {
    ...
    expect(RESULT).toEqual(QuotaQuantity.create("425825.142804"));
  });
  ...
  it("returns the same quota quantity when only one value is provided", () => {
    const RESULT = calculatePortfolioWithdrawalQuotasSum({
      quotaQuantity: [{ value: QuotaQuantity.create("225825.142804") }],
    });

    expect(RESULT).toEqual(QuotaQuantity.create("225825.142804"));
  });
