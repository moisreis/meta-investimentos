...
import { calculateWithdrawalQuotasSum } from "@/business/calculators/position/withdrawal-quotas-sum.calculator";
...
describe("calculateWithdrawalQuotasSum", () => {
  it("returns the sum of multiple quota quantities", () => {
    const RESULT = calculateWithdrawalQuotasSum({
      quotaQuantity: [
        { value: QuotaQuantity.create("225825.442801") },
        { value: QuotaQuantity.create("201000.000000") },
      ],
    });

    expect(RESULT).toEqual(QuotaQuantity.create("426825.442801"));
  });
