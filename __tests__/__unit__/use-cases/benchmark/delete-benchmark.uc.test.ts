...
describe("deleteBenchmark", () => {
  ...
  describe("success", () => {
    it("deletes a benchmark", async () => {
      ...
      await deleteBenchmark(unitOfWork as never, {
        actorId: MANAGER_ID,
        benchmarkId: ID.BENCHMARK.DEFAULT,
      });

      const deleted = await unitOfWork.benchmarks.findById(
        EntityId.create(ID.BENCHMARK.DEFAULT),
      );
  ...
  describe("authorization", () => {
    it("throws NotFoundError when the benchmark does not exist", async () => {
      ...
      await expect(
        deleteBenchmark(unitOfWork as never, {
          actorId: MANAGER_ID,
    ...
    it("does not delete the benchmark when actor is not a manager", async () => {
      unitOfWork.seed({ users: [MANAGER], benchmarks: [BENCHMARK] });
      ...
      const still = await unitOfWork.benchmarks.findById(
        EntityId.create(ID.BENCHMARK.DEFAULT),
      );
