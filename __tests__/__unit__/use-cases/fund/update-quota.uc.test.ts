...
describe("updateQuota", () => {
  ...
  describe("success", () => {
    it("updates the price of a quota", async () => {
      unitOfWork.seed({ users: [MANAGER], quotas: [QUOTA] });

      const RESULT = await updateQuota(unitOfWork as never, {
        actorId: MANAGER_ID,
      ...
      const saved = await unitOfWork.quotas.findById(
        EntityId.create(ID.QUOTA.DEFAULT),
      );
      expect(saved?.price.value.toString()).toBe("1500");
    });

    it("attributes the mutation to the actor", async () => {
      unitOfWork.seed({ users: [MANAGER], quotas: [QUOTA] });

      await updateQuota(unitOfWork as never, {
        actorId: MANAGER_ID,
        quotaId: ID.QUOTA.DEFAULT,
        price: "1500.00",
      });
  ...
  describe("authorization", () => {
    it("throws NotFoundError when the quota does not exist", async () => {
      ...
      await expect(
        updateQuota(unitOfWork as never, {
          actorId: MANAGER_ID,
