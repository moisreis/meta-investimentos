...
describe("deleteNorm", () => {
  ...
  describe("success", () => {
    it("deletes a norm", async () => {
      unitOfWork.seed({ users: [MANAGER], norms: [NORM] });

      await deleteNorm(unitOfWork as never, {
        actorId: MANAGER_ID,
        normId: ID.NORM.DEFAULT,
      });

      const deleted = await unitOfWork.norms.findById(
  ...
  describe("authorization", () => {
    it("throws NotFoundError when the norm does not exist", async () => {
      ...
      await expect(
        deleteNorm(unitOfWork as never, {
          actorId: MANAGER_ID,
    ...
    it("does not delete the norm when actor is not a manager", async () => {
      unitOfWork.seed({ users: [MANAGER], norms: [NORM] });
      ...
      const still = await unitOfWork.norms.findById(
        EntityId.create(ID.NORM.DEFAULT),
      );
